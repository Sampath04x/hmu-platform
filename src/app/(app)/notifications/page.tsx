...
function NotifIcon({ type }: { type: string }) {
  ...
  switch (type) {
    case "match":
      return <div className={cn(base, "bg-indigo-500/15 border border-indigo-500/20")}><MessageSquareIcon className="w-5 h-5 text-indigo-400" /></div>;
    case "club":
      return <div className={cn(base, "bg-violet-500/15 border border-violet-500/20")}><BellIcon className="w-5 h-5 text-violet-400" /></div>;
    case "person":
  ...
  return (
    <div className="min-h-screen bg-background pb-24 max-w-2xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-3xl font-sora font-semibold text-white">Notifications</h1>
        <span className="ml-auto text-xs text-indigo-400 hover:text-indigo-300 cursor-pointer font-medium">Mark all read</span>
      </div>
            ...
            {items.map((notif) => (
              <Link key={notif.id} href={notif.href}>
                <div className={cn(
                  ...
                  notif.unread
                    ? "bg-indigo-500/5 border border-indigo-500/10 hover:border-indigo-500/25 hover:bg-indigo-500/10"
                  ...
                  {notif.unread && (
                    <div className="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_6px_rgba(99,102,241,0.7)] shrink-0" />
                  )}
                </div>
              </Link>
            ))}
