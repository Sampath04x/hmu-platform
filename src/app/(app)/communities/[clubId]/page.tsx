...
export default function ClubPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      {/* Cover Banner */}
      <div className="h-48 md:h-64 w-full bg-gradient-to-r from-indigo-500/20 via-violet-500/20 to-emerald-500/20 relative">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 relative -mt-16 md:-mt-20">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
          <div className="flex items-end gap-5">
            <div className="w-28 h-28 md:w-36 md:h-36 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-500 text-white font-bold text-4xl md:text-5xl flex items-center justify-center border-[6px] border-background shadow-2xl shrink-0">
              {club.initials}
            </div>
            ...
            <div className="mb-2 space-y-2">
              <div className="flex items-center gap-2">
                ...
                {club.verified && (
                  <Badge variant="outline" className="border-blue-500/30 text-blue-400 gap-1 rounded-md">
                    <CheckCircle2Icon className="w-3.5 h-3.5" /> Verified
                  </Badge>
                )}
              </div>
              <h1 className="text-3xl md:text-4xl font-sora font-bold text-white leading-tight">{club.name}</h1>
          ...
          <div className="flex gap-3 mb-2 shrink-0">
            <Button className="w-full md:w-32 bg-indigo-500 hover:bg-indigo-600 text-white font-semibold shadow-[0_0_15px_rgba(99,102,241,0.3)] transition-all">
              Join Club
            </Button>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <Card className="p-5 bg-card border-border/50 glow-hover">
                <div className="flex gap-3 mb-4">
                  <div className="w-10 h-10 rounded-lg bg-indigo-500/20 text-indigo-400 font-bold flex items-center justify-center text-sm">{club.initials}</div>
                  <div>
                  ...
                  <Badge className="ml-auto h-6 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 hover:bg-indigo-500/20" variant="outline">
                    EVENT
                  </Badge>
                </div>
                ...
                <div className="w-full h-48 bg-muted rounded-xl bg-gradient-to-br from-indigo-500/10 to-transparent flex items-center justify-center text-muted-foreground border border-border/50">
                  [Image Placeholder]
                </div>
              </Card>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
              {[1, 2].map((i) => (
                <Card key={i} className="overflow-hidden bg-card border-border/50 glow-hover flex flex-col sm:flex-row group transition-all">
                  <div className="w-full sm:w-48 h-32 sm:h-auto bg-gradient-to-br from-indigo-500/20 to-violet-500/10 group-hover:from-indigo-500/30 transition-colors flex items-center justify-center shrink-0 border-r border-border/30">
                    <CalendarIcon className="w-8 h-8 text-indigo-400/50" />
                  </div>
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-1">
                        <Badge className="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 hover:bg-indigo-500/20 mb-2">SAT, 6:00 AM</Badge>
                        <span className="text-xs text-muted-foreground font-medium bg-muted px-2 py-1 rounded-md">34 Going</span>
                      </div>
                      <h3 className="text-lg font-sora font-semibold text-white group-hover:text-indigo-400 transition-colors">Weekly Campus Photowalk</h3>
                      <p className="text-sm text-muted-foreground mt-1 flex items-center gap-1.5"><MapPinIcon className="w-4 h-4" /> Main Gate</p>
                    </div>
                    <div className="mt-4 flex gap-2">
                      <Button size="sm" className="bg-indigo-500 hover:bg-indigo-600 text-white font-semibold flex-1 shadow-[0_0_10px_rgba(99,102,241,0.2)]">Going</Button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
              {Array(8).fill(0).map((_, i) => (
                <Card key={i} className="p-4 flex flex-col items-center justify-center text-center bg-card border-border/50 hover:bg-card/80 transition-colors cursor-pointer group">
                  <Avatar className="w-16 h-16 mb-3 border-2 border-transparent group-hover:border-indigo-500/50 transition-colors shadow-lg">
                    <AvatarFallback className="bg-indigo-500/10 text-indigo-400 font-bold text-lg">P{i}</AvatarFallback>
                  </Avatar>
          ...
          {activeTab === "About" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <Card className="p-6 bg-card border-border/50">
                <h3 className="text-lg font-sora font-semibold mb-3">About {club.name}</h3>
              ...
              <Card className="p-6 bg-card border-border/50">
                <h3 className="text-lg font-sora font-semibold mb-3">Links</h3>
                <div className="space-y-3">
                  <a href="#" className="flex items-center gap-3 text-indigo-400 hover:text-indigo-300 transition-colors group">
                    <LinkIcon className="w-4 h-4 bg-indigo-500/10 p-1 rounded-md box-content group-hover:bg-indigo-500/20" /> 
